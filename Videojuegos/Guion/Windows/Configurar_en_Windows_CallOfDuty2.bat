...
REM |--------------------------------------------------------------------------|
SET ARCHIVO=\Setup\Data\COD2SP_s.exe
...
if not exist "%CARPETAESCRITORIO%\Call Of Duty 2 - Un Solo Jugador.lnk" goto make_codsp
...
:make_codsp
echo "Call Of Duty 2 - Un Solo Jugador"
set SCRIPT="%TEMP%\%RANDOM%-%RANDOM%-%RANDOM%-%RANDOM%.vbs"
echo Set oWS = WScript.CreateObject("WScript.Shell") >> %SCRIPT%
echo sLinkFile = "%CARPETAESCRITORIO%\Call Of Duty 2 - Un Solo Jugador.lnk" >> %SCRIPT%
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> %SCRIPT%
echo oLink.TargetPath = "%PWD%\CoD2SP_s.exe" >> %SCRIPT%
...
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> %SCRIPT%
echo oLink.TargetPath = "%PWD%\CoD2MP_s.exe" >> %SCRIPT%
echo oLink.WorkingDirectory = "%PWD%" >> %SCRIPT%
echo oLink.Save >> %SCRIPT%
cscript /nologo %SCRIPT%
del %SCRIPT%
