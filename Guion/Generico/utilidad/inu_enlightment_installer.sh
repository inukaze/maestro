...
# del Escritorio Enlightment , a la fecha -> 09 de Septiembre del 2013
#
...
if [ ! -d /opt/desktop/enlightenment ]; then
sudo mkdir -p /opt/desktop/enlightenment
sudo mkdir -p /opt/desktop/enlightenment/bin
sudo mkdir -p /opt/desktop/enlightenment/lib
fi
...
#Determinar la ultima version disponible en internet
#De la libreria "efl"

EFLV=$(curl -s "https://download.enlightenment.org/rel/libs/efl/" | grep -Po "efl-[0-9].{0,9}.[0-9].tar.gz" | tail -n 1)
wget https://download.enlightenment.org/rel/libs/efl/$EFLV
tar xfvz $EFLV
