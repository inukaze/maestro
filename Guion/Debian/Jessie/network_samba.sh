...
FECHA=$(date +"%Y-%m-%d")
HORA=$(date +"%r")
ADC="/etc/samba/smb.conf"
sudo echo -e '\nSe instalara : Samba , para poder Compartir con Redes Microsoft Windows\n' ; \
sudo apt-get -y install samba gvfs-fuse libsmbclient smbclient python-smbc xinetd ; \
echo -e '\nSe creara el directorio : "/var/lib/samba/usershares/"\n' ; \
sudo mkdir -p "/var/lib/samba/usershares/" ; \
echo -e '\nSe agregara al usuario '"$USER"' al grupo "sambashare"\n' ; \
sudo gpasswd -a "$USER" sambashare ; \
echo -e '\nEstablece la contraseña de Samba para el Usuario : '"$USER"'\n' ; \
sudo smbpasswd -a "$USER" ; \
echo -e '\nSe establecieron los demonios Samba & Netbios\n"' ; \
sudo systemctl enable smbd.service ; sudo systemctl start smbd.service ; \
echo -e '\n Se iniciaran los demonios Samba & Netbios\n' ; \
sudo systemctl start nmbd.service ; sudo systemctl start nmbd.service ; \
...
echo '# Archivo : '"$ADC"'
# de Inukaze (De Venezuela)
#
# Sitio Web -> http://https://goo.gl/ij6WqW
#
# Este es mi archivo de configuracion para Samba
# lo uso con Xfce para que me permita comandamente
# Des/Compartir Carpetas en Intranet.
#
# Instalar en ArchLinux :
# sudo pacman -Sy --noconfirm  samba fusesmb  smbclient python2-pysmbc python-pysmbc gvfs-smb xinetd
#
# Instalar en Debians : 
# sudo apt-get -y install samba gvfs-fuse libsmbclient smbclient python-smbc xinetd
#
...
# sudo systemctl start nmbd.service ; sudo systemctl start nmbd.service
#
# Configurando las acciones Personalizadas de Thunar
...
#
# Nombre : ——–  Elementos Compartidos  ——–
# Una vez agregadas todas , ya tengo en el Sub-Menu desplegable las opciones
# Para Des/Compartir Rapida y Comandamente las carpetas que quiero , sin la
# Necesidad de agregar una por una en el archivo de configuracion de samba (smb.conf)
#
...
;       write list = root, @lpadmin' | sudo tee "$ADC"
